(ns jis_standards.main
  "Kotodama WASM entrypoint for the Jis_standards clean-room actor (L4) — Clojure port.
  ...
  facts (`jis_standards.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "jis_standards")
(def tier "L4")
...
(def entity-specs
  [{:entity "Substance" :plural "substances" :id-prefix "jisstand_sub"
    :fields [:casNumber :name :formula :molarMass] :required [:casNumber :name]
    :coerce {:molarMass :float} :refs {}}
   {:entity "Compound" :plural "compounds" :id-prefix "jisstand_com"
    :fields [:inchiKey :smiles :name] :required [:inchiKey :smiles]
    :coerce {} :refs {}}
   {:entity "Structure" :plural "structures" :id-prefix "jisstand_str"
    :fields [:pdbId :title :method :resolution] :required [:title :method]
    :coerce {:resolution :float} :refs {}}
   {:entity "Material" :plural "materials" :id-prefix "jisstand_mat"
    :fields [:materialId :formula :bandGap] :required [:formula :bandGap]
    :coerce {:bandGap :float} :refs {}}
   {:entity "Standard" :plural "standards" :id-prefix "jisstand_sta"
    :fields [:designation :title :organization] :required [:designation :title]
    :coerce {} :refs {}}
   {:entity "Property" :plural "properties" :id-prefix "jisstand_pro"
...
(defn emit-facts
  "EAVT facts for one record: {\"jis_standards.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jis_standards-compat" :tier tier :entities entities} 200])
...
(defn start! [] :jis_standards-compat/ready)
